...
#
#     # --nodebump: The structure generated by pdb2pqr with incomplete gaps will cause an error the next time pdb2pqr is run with it as the input file.
#     pdb2pqr_cmd = ["pdb2pqr", "--nodebump", "--protonate-all", "--ff=AMBER", "--log-level", "CRITICAL", "--titration-state-method", "propka",
...
#
#
# def calc_hbond(pdb_h):
#     hbonds = []
#     stack_h = strucio.load_structure(pdb_h)
#     triplets_h = hbond(stack_h)  # output from add_h structure with different atom number but same residue num
#     donor_acceptor = [[list(map(lambda x: x + 1, list(bond[::2]))), list(stack_h[bond].res_id[::2])] for bond in
#                       triplets_h]
#     for pair in donor_acceptor:
#         atom_id1 = int(pair[0][0])
#         res_id1 = int(pair[1][0])
#
#         atom_id2 = int(pair[0][1])
#         res_id2 = int(pair[1][1])
#         bond = {(atom_id1, res_id1), (atom_id2, res_id2)}
#         if bond not in hbonds:
#             hbonds.append(list(bond))
#
#     return hbonds
#
#
...
#     hbond_results = calc_hbond(pdb_addh_temp_file_path)
#     return hbond_results
#
#
# if __name__ == '__main__':
#     structure_path = r"/Users/douzhixin/Developer/qProtein2/Data/pdb/1yna.pdb"
#     ret = run(structure_path)
#     print(ret)
#     res = [str(group[1]) for pair in ret for group in pair]
#     print('sele res ' + '+'.join(res))
...
#
# Description:
#   Unified service for H-bond analysis using PDB2PQR (add H)
#   and Biotite hydrogen bond geometry calculation.
#
#   Supports:
#       - return H-bond frequency (old behavior)
#       - return exact donor/acceptor residue pairs (new behavior)
#
#   All previous code versions have been merged into this file.
# ------------------------------------------------------------------------------
