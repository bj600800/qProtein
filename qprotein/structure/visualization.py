...
def get_pae_plddt(model_names):
    ...
    for i, name in enumerate(model_names):
        ...
        out[f'model_{i + 1}'] = {'plddt': d['plddt'],
                                 'pae': d['predicted_aligned_error'], 'distogram': d['distogram']}
    return out
...
def generate_output_images(feature_dict, out_dir, name, pae_plddt_per_model):
    ...
    ##################################################################
    num_models = 5
    plt.figure(figsize=(3 * num_models, 2), dpi=300)
    for n, (model_name, value) in enumerate(pae_plddt_per_model.items()):
        plt.subplot(1, num_models, n + 1)
        plt.title('')
        plt.imshow(value["pae"], label=model_name, cmap='Greens_r', vmin=0, vmax=35)
        plt.colorbar()
...
def main():
    ...
    model_names = [f'{args.input_dir}/result_model_{f}{"_multimer" if is_multimer else "_ptm" if is_ptm else ""}.pkl' for f
                   in range(1, 6)]
