...
from qprotein.structure.afdbStructureSegmenter import Segmenter
...
class TargetNameRetriever(SqlSearch):
	...
	def get_sql_results(self):
		...
		# sql_cmd = "SELECT query_name, query_length, sprot_acc, sprot_subject_start, sprot_subject_end," \
		#           "trembl_acc, trembl_subject_start, trembl_subject_end  " \
		#           "FROM results_summary WHERE sprot_acc != '' or trembl_acc != ''"
		# sql_cmd = "SELECT query_name, query_length, sprot_acc, sprot_subject_start, sprot_subject_end " \
		# 		  "FROM results_summary WHERE sprot_acc != ''"
		sql_cmd = "SELECT query_name, query_length, trembl_acc, trembl_subject_start, trembl_subject_end " \
				  "FROM results_summary WHERE trembl_acc != ''"
	...
	def get_exist_query(self):
		exist_structure = [file for path in self.structure_dir_path for file in os.listdir(path)
						   if
						   os.path.isfile(os.path.join(path, file)) and os.path.getsize(os.path.join(path, file)) > 0]
	...
	def filter_results(self):
		...
		for result in results_list:
			if result[2] is not None:
				query_info.append((result + ("sprot",)))
...
class Consumer(threading.Thread):
	...
	def map_structure(self, cif_str, query_info):
		query_name, query_length, uniprot_acc, subject_start_residue, subject_end_residue, uniprot_class = query_info
		
		write_cif_path_kw = [self.structure_dir_path, query_name, uniprot_acc]
		segmenter = Segmenter(
			cif_string=cif_str, query_name=query_name, subject_start_residue=subject_start_residue,
			subject_end_residue=subject_end_residue, query_length=query_length,
			write_cif_path_kw=write_cif_path_kw, uniprot_class=uniprot_class
			)
		try:
			output_cif, mean_plddt = segmenter.run()
			write_cif_path = self.write_cif(
				output_cif=output_cif, mean_plddt=mean_plddt, uniprot_class=uniprot_class,
...
def map_multiprocess(task_dir):
	...
	target_handler = TargetNameRetriever(
		summary_results_db=summary_results_db, structure_dir_path=structure_dir_path,
		log_file=log_file
		)
	...
	if query_info:
		...
		for i in range(producer_thread):
			t = Producer(
				query_info_queue=query_info_queue, structure_info_queue=structure_info_queue, log_file=log_file
				)
		...
		for i in range(consumer_thread):
			t = Consumer(
				query_info_queue=query_info_queue, structure_info_queue=structure_info_queue,
...
if __name__ == '__main__':
	task_dir = r"D:\subject\active\1-qProtein\data\enzymes\cazy_endo-1_4-beta-xylanase"
